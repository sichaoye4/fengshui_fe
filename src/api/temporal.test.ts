...
describe("fetchGregorianConversion", () => {
  it("sends date and returns parsed response", async () => {
    ...
    const result = await fetchGregorianConversion("2026-04-25");
    expect(result).toEqual(expected);
    expect(globalThis.fetch).toHaveBeenCalledWith(
      "/api/v1/temporal/convert/gregorian?date=2026-04-25",
  ...
  it("includes optional time parameter", async () => {
    mockFetchOk({});
    await fetchGregorianConversion("2026-04-25", "14:30:00");
    expect(globalThis.fetch).toHaveBeenCalledWith(
  ...
  it("throws on HTTP error", async () => {
    ...
    await expect(fetchGregorianConversion("bad")).rejects.toThrow("Invalid date");
  });

  it("throws generic on HTTP error without detail", async () => {
    mockFetchError(500);
    await expect(fetchGregorianConversion("boom")).rejects.toThrow("HTTP 500");
  });
});
...
describe("fetchAnnualTemporal", () => {
  it("sends year ganzhi and returns result", async () => {
    ...
    mockFetchOk(expected);
    const result = await fetchAnnualTemporal("Jia-Chen");
    expect(result).toEqual(expected);
    expect(globalThis.fetch).toHaveBeenCalledWith(
      "/api/v1/temporal/annual?year_ganzhi=Jia-Chen",
      expect.any(Object)
    );
  });
});
...
describe("fetchMonthlyTemporal", () => {
  it("sends year/month ganzhi", async () => {
    mockFetchOk({});
    await fetchMonthlyTemporal("Jia-Chen", "Ding-Si");
    expect(globalThis.fetch).toHaveBeenCalledWith(
      "/api/v1/temporal/monthly?year_ganzhi=Jia-Chen&month_ganzhi=Ding-Si",
      expect.any(Object)
    );
  });
});
...
describe("fetchAnnualFlyingStar", () => {
  it("sends solar year as number", async () => {
    mockFetchOk({});
    await fetchAnnualFlyingStar(2026);
    expect(globalThis.fetch).toHaveBeenCalledWith(
      "/api/v1/temporal/flying-star/annual?solar_year=2026",
      expect.any(Object)
    );
  });
});
...
describe("fetchFourYunProfile", () => {
  it("sends solar year and sitting bagua", async () => {
    mockFetchOk({});
    await fetchFourYunProfile(2026, "KAN");
    expect(globalThis.fetch).toHaveBeenCalledWith(
      "/api/v1/periods/four-yun?solar_year=2026&sitting_bagua=KAN",
      expect.any(Object)
    );
  });
});
...
describe("fetchLiqiHouseProfile", () => {
  it("sends sitting bagua in path", async () => {
    ...
    mockFetchOk(expected);
    const result = await fetchLiqiHouseProfile("QIAN");
    expect(result).toEqual(expected);
    expect(globalThis.fetch).toHaveBeenCalledWith(
      "/api/v1/liqi/house/QIAN",
      expect.any(Object)
    );
  });
});
