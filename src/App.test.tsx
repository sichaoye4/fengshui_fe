  ...
  const fetchMock = vi.fn((input: RequestInfo | URL) => {
    ...
    if (url.includes("/api/v1/temporal/convert/gregorian")) {
      ...
      body = {
        ...
        bagua_star: { ä¹? 2, å…? 3, è‰? 4, ç¦? 5, å? 6, å? 7, éœ? 8, å·? 9, ä¸? 1 }
      };
      ...
      body = {
        ...
        qi_type_labels: { SHENG_QI: { zh: "ç”Ÿæ°”", en: "Sheng Qi" } },
        five_qi_palaces: {},
        five_qi_directions: {},
      ...
      body = {
        ...
        building_facing_bagua: "éœ?,
        building_facing_bagua_code: "ZHEN",
        building_facing_bagua_zh: "éœ?,
        door_bagua: "å?,
        door_bagua_code: "KUN",
        door_bagua_zh: "å?,
        door_sitting_bagua: "è‰?,
        door_sitting_bagua_code: "GEN",
        door_sitting_bagua_zh: "è‰?,
        base_bagua: "éœ?,
      ...
      body = {
        house_analysis: {
          status: "ok",
          attributes: {
            sitting: { bagua: "KAN", bagua_zh: "å?, element: "WATER" },
          ...
          decade_analyses: [
            {
              ...
              years_end: 10,
              lord_element: "WATER",
              diagnosis: { type: "strong_with_wealth", en: "Body strong with wealth", zh: "èº«æ—ºæœ‰è´¢" },
              affliction: null,
              pathogen: null
            },
            {
              ...
              lord_element: "WATER",
              diagnosis: null,
              affliction: { type: "pathogen_present", type_zh: "ç—…åŽŸåœ¨è¿" },
              pathogen: { bagua: "KUN", bagua_zh: "å?, element: "EARTH" }
            },
            {
              decade_index: 6,
              years_range: "51-60",
              ...
              diagnosis: null,
              affliction: { type: "pathogen_present", type_zh: "ç—…åŽŸåœ¨è¿" },
              pathogen: { bagua: "LI", bagua_zh: "ç¦?, element: "FIRE" }
            }
          ],
          door_analysis: null,
          overall_summary: {
            ...
            has_affliction: true,
            summary_zh: "é™å®…æœ‰ä¸€æ®µç—…åŽŸåœ¨è¿?,
          ...
          persons: [
            {
              ...
              birth_year: 1994,
              gender: "male",
              minggua: { bagua: "QIAN", bagua_zh: "ä¹?, group: "è¥¿å››å‘? },
              status: "ok",
              is_affected: true,
              ...
              affected_decades: [
                {
                  ...
                  matching_categories: [{ category_zh: "ç»å‘½", label_zh: "ç»å‘½" }],
                  person_bagua: "QIAN",
                  person_bagua_zh: "ä¹?
                }
              ],
              summary_zh: "å—é™å®…è¿ç¨‹å½±å“?
            }
          ]
        }
      };
      ...
      body = {
        ...
        gender: "male",
        gender_zh: "ç”?,
        person_minggua: { bagua_code: "QIAN", bagua_zh: "ä¹?, group_en: "West Four" },
        house_bagua: "KAN",
        house_bagua_code: "KAN",
        house_bagua_zh: "å?,
        house_group: { group_en: "East Four" },
        group_match: false,
        star_relation: { relation_en: "Liu Sha", relation_zh: "å…­ç…ž", tier: "medium" },
        overall_is_auspicious: false,
        overall_label_zh: "ä¸å‰",
        overall_label_en: "inauspicious"
      };
    }
...
describe("App tabbed workflow", () => {
  it("renders simplified chinese labels when language is switched", async () => {
    ...
    await user.click(screen.getByRole("button", { name: /ä¸­æ–‡|æ¶“î…Ÿæž? }));

    expect(screen.getByText("è¾“å…¥å·¥ä½œåŒ?)).toBeInTheDocument();
    expect(screen.getByRole("tab", { name: /å®…å‘½ç†æ°”|ç€¹å‘­æ‡¡éžå—˜çšµ/ })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: /æ‰§è¡Œè¯„ä¼°|éŽµÑ†î”‘ç’‡å‹ªå? })).toBeInTheDocument();
  });
  ...
  it("shows flow year pillar from selected gregorian date/time", async () => {
    ...
    expect(flowYearPillarValue.textContent).not.toBe("éˆ?");
  });
  ...
  it("renders bagua selectors as chinese + code in zh mode and omits incoming sha element", async () => {
    ...
    const sittingBaguaSelect = screen.getByLabelText(/é§æ„¬å´·|åå¦/) as HTMLSelectElement;

    expect(within(sittingBaguaSelect).getByRole("option", { name: /QIAN/ })).toBeInTheDocument();
    expect(screen.getByLabelText("¼äÊý")).toBeInTheDocument();
    expect(screen.queryByLabelText(/Incoming Sha Element|ÃÅÖ®ÎåÐÐ/)).not.toBeInTheDocument();
